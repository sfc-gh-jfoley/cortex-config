...
`sv-ddl` / `sv-iterative-optimizer` / `agent-optimizer`):

- **INTERACTIVE** (default): the `MANDATORY STOP` in Phase 1 and the `STOPPING POINT` in
  Phase 5 both block for user input.
- **AUTONOMOUS**: Phase 1 logs the context summary and proceeds. Phase 5 proceeds
  ('go') automatically once self-check shows 0 failing rules; if warnings remain,
  apply the skill's own suggested fix for each (Phase 5's `edit N` guidance already
  states the fix) and re-run self-check once before proceeding — do not loop
  indefinitely on warnings.

Default to INTERACTIVE if unclear. A standing "run to completion" directive for this task
counts as an explicit request for AUTONOMOUS.
...
5. **Eval handoff built in**: Phase 7 writes `handoff.json` and offers to invoke `agent-model-tester`, `agent-optimizer`, the bundled evaluate workflow, or CI/CD deployment (Phase 8).
...
| Goal | Skill to use |
|------|-------------|
| New agent — pure SQL, HOL, auto-descriptions, self-check | **This skill** (agent-ddl) |
...
| Model/flag variant comparison | `agent-model-tester` |
| Iterative improvement loop with DEV/TEST split | `agent-optimizer` |
| Experimental flags reference (EnableAgenticAnalyst etc.) | `agent-flags-reference` |
