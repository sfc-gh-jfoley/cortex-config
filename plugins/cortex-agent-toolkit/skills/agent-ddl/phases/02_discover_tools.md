---
name: agent-ddl-phase2-discover-tools
...
## ⚠️ MANDATORY STOP (INTERACTIVE mode — see AUTONOMOUS resolution below)
...
Wait for user approval before loading Phase 3 (INTERACTIVE mode). **AUTONOMOUS:** accept the
generated descriptions where "Description quality: N/N passed" (all checks pass); log
and proceed. If any description failed quality checks, regenerate once per this file's
own remediation note above, then proceed regardless — do not block on wording.
