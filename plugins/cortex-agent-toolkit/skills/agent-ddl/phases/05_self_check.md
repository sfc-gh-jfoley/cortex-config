---
name: agent-ddl-phase5-self-check
...
| | **Exception — no-resource tools**: `web_search`, `data_to_chart`, `code_execution`, and `code_toolset_all` tools do NOT require a `tool_resources` entry. If these tool types appear in the `tools` array without a matching `tool_resources` key, this is correct — do not flag as an error. Note: `code_execution` MAY have a `tool_resources` entry (for `permission_policy`, `artifact_repositories`, or `external_access_integrations`) — this is also valid and must not be flagged. | | |
...
| 18 | Event table configured when agent uses observability spans | For each `cortex_analyst_text_to_sql` tool: get the `semantic_view` FQN from `tool_resources`. Run `SELECT GET_DDL('semantic_view', '<SV_FQN>')` and check if the DDL references any physical table whose name contains `SPANS`, `OBSERVABILITY`, or `AI_OBSERVABILITY`. If yes, run `SHOW PARAMETERS LIKE 'EVENT_TABLE' IN ACCOUNT` and check that the `value` column is non-empty. | WARN — "This agent's SV includes an observability spans table (`<table_name>`), but no account-level event table is configured. Spans data will be empty until you run: `ALTER ACCOUNT SET EVENT_TABLE = <db.schema.table>`. The agent and SV will deploy successfully, but tool latency and span-based queries will return 0 rows." Offer: (A) Configure event table now (user must provide the FQN), (B) Dismiss — I'll configure it later |
...
⚠️ **STOPPING POINT (INTERACTIVE mode)** — Do not proceed to Phase 6 until the user types 'go'.

**AUTONOMOUS:** proceed automatically once 0 rules are failing (per `SKILL.md` Execution
Mode) — apply the suggested fix for any remaining warnings, re-check once, then proceed
regardless of residual warnings.
