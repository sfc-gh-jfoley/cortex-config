...
> `agent-ddl` (create/alter), `agent-evaluation` (evaluate),
> `agent-model-tester` (flag and model variants), `agent-optimizer`
...
**Skill**: `agent-ddl` (cortex-agent-toolkit plugin)
...
**Skill**: `agent-model-tester` (cortex-agent-toolkit plugin)
...
**Skill**: `agent-optimizer` (cortex-agent-toolkit plugin)
...
2-3 consecutive rejections means you've hit a local optimum. Before escalating to GEPA, try the DSPy instruction optimizer — it explores the joint (instruction × demonstration) space and often breaks local optima that sequential iteration cannot. Escalate to Phase 6 (GEPA) only if DSPy compilation also fails to improve.

**Escalation order:**
1. Iterative optimizer (Phase 5) — sequential, targeted, fast
2. **DSPy MIPROv2 optimizer** (`agent-instruction-optimizer`) — joint instruction + few-shot compilation via TPE; invoke when Phase 5 hits 2-3 rejections
3. GEPA (Phase 6) — evolutionary population search; invoke when DSPy also converges below target

---

## 6.5. Phase 5.5: DSPy Instruction Compilation

**Skill**: `agent-instruction-optimizer` (cortex-agent-toolkit plugin)

### When to Use

- Phase 5 (iterative) hit 2-3 consecutive rejections on the same failures
- Before committing to the full GEPA setup (metadata.yaml, population scaffolding)
- Budget: ~20 eval calls

### What It Does

Implements Stanford's MIPROv2: bootstraps demonstrations from baseline eval results,
proposes 12 instruction variants via targeted mutation operators, then uses optuna TPE
to jointly search over (instruction variant × few-shot demo subset). A real surrogate
model — not random search.

### Outcome Gates

- **Improved AND above target**: accept, skip Phase 6, go to Phase 7
- **Improved but below target**: accept as new baseline, proceed to Phase 6 with winning instructions as starting point
- **No improvement**: discard, proceed to Phase 6 with original instructions
...
**Skill**: `agent-flags-reference` (cortex-agent-toolkit plugin)
...
2. Check the `agent-flags-reference` skill reference for updates
...
|-------|--------|-------------|------------|
| `agent-ddl` | cortex-agent-toolkit | Create/edit agents | `create_or_alter_agent.py` |
| `agent-evaluation` | cortex-agent-toolkit | Run formal evals | `run_evaluation.py` |
| `agent-model-tester` | cortex-agent-toolkit | Compare flag variants | (stage configs + `EXECUTE_AI_EVALUATION`) |
| `agent-optimizer` | cortex-agent-toolkit | Iterative instruction tuning | `build_agent_spec.py` |
| `agent-gepa-optimizer` | cortex-agent-toolkit | Evolutionary optimization | `mutate.py`, `tournament.py` |
| `agent-flags-reference` | cortex-agent-toolkit | Flag reference | (docs search) |
| `agent-query` | cortex-agent-toolkit | Programmatic testing | `test_agent.py` |
...
Phase 5 (Iterative Optimize)
  │
  ▼
Converged? (2-3 rejections) ──Yes──▶ Phase 5.5 (DSPy MIPROv2)
  │                                       │
  No (still improving)             Improved?
  │                                  │         │
  │                                 Yes        No
  │                                  │         │
  │                                  ▼         ▼
  │                          Phase 7      Phase 6 (GEPA)
  │                       (Validate)          │
  │                                           ▼
