...
metrics:
  - name: "answer_correctness"       # pin version — bare/auto can resolve to a legacy judge (v1/claude-4-sonnet); see agent-evaluation/references/eval-troubleshooting.md #8
    version: "v3"
  - name: "logical_consistency"
    version: "v3"
  - name: "factual_correctness_verdict"
    model: "claude-sonnet-4-6"  # explicit — omitting this defaults to legacy claude-4-sonnet and can fail on accounts with no prior usage of that model
    score_ranges:
      min_score: [0, 0]
      median_score: [1, 1]
      max_score: [1, 1]
...
- `tool_selection_accuracy` — **custom metric** (replaces the deprecated built-in). Evaluates whether the agent selected the correct tool(s) in the correct sequence. Scored 0-10. See `eval-config-template.yaml` in agent-model-tester references for the prompt definition. Include in YAML as a named custom metric block, not as a bare string.
