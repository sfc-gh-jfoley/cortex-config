...
**STOP GATE (INTERACTIVE):** Confirm the variant matrix with the user before proceeding. **AUTONOMOUS:** run the model comparison (3 variants) — the matrix's own stated default for a first sweep — and log the selection.

### Step 2: Deploy Agent Variants

For each row in the matrix, create (or verify) an agent in the target schema.

> **Rollback gate (INTERACTIVE):** Ask the user: "Want me to create a rollback clone first so we can undo this?" then execute on confirmation. **AUTONOMOUS:** create the rollback clone (the safer default for mutating DDL) and execute — no wait.
...
**STOP GATE (INTERACTIVE):** Show the deploy SQL for each variant and ask for confirmation. **AUTONOMOUS:** already covered by the rollback gate above — execute without a second wait.
...
**STOP GATE (INTERACTIVE):** Show config summary (variants x splits x runs = total files). **AUTONOMOUS:** log the summary, fire evals — compute cost, not a judgment call.
...
**7b: INSERT per-question scores into manifest and cross-agent history**
...
SELECT INPUT_ID, INPUT, METRIC_NAME, EVAL_AGG_SCORE,
    'flag_sweep',
    'FLAG_SWEEP',
    NULL,  -- flag sweeps are standalone; no OPT_RUNS entry
    '<RUN_PREFIX>_{variant}_{split}_r{N}',
    '<VARIANT>',
    NULL,
...
Also insert into cross-agent history if `opt_history_fqn` is set in `metadata.yaml`
(load INSERT pattern from `../../../../references/pattern-mining.md`):
```sql
...
SELECT
    '<DATABASE>.<SCHEMA>.<AGENT_NAME>', '<SV_FQN>', 'FLAG_SWEEP', NULL,
    NULL, NULL,
    INPUT_ID, INPUT, METRIC_NAME, EVAL_AGG_SCORE,
    NULL,  -- no baseline_score for flag sweep
    'flag_sweep', '<RUN_PREFIX>_{variant}_{split}_r{N}', TEST_CATEGORY
FROM TABLE(SNOWFLAKE.LOCAL.GET_AI_EVALUATION_DATA(
    '<DATABASE>', '<SCHEMA>', '<AGENT_NAME>_{SUFFIX}', 'CORTEX AGENT',
    '<RUN_PREFIX>_{variant}_{split}_r{N}'
))
...
**STOP GATE (INTERACTIVE):** Ask the user which variant to promote (or none).

**AUTONOMOUS:** promote the Step 4/scoring-logic recommended winner above (primary +
tiebreakers already produce a deterministic pick) unless the margin over the original
is negligible, in which case promote none and log "no clear winner — kept original."
