...
CREATE TABLE IF NOT EXISTS <DATABASE>.<SCHEMA>.<AGENT_NAME>_QUESTION_MANIFEST (
    ...
    SOURCE         VARCHAR      NOT NULL,   -- 'flag_sweep' | 'optimize_dev' | 'optimize_test' | 'gepa_gen' | 'tpe_trial' | 'tpe_final' | 'baseline' | 'revalidation'
    OPTIMIZER_TYPE VARCHAR,                  -- 'ITERATIVE' | 'GEPA' | 'INSTRUCTION_TPE' | 'FLAG_SWEEP' | NULL for baselines
    RUN_ID         VARCHAR,                  -- UUID from OPT_RUNS — join to get full run context
    RUN_LABEL      VARCHAR      NOT NULL,   -- e.g. 'FASTPATH_OFF_dev_r1', 'iter3_dev_r2', 'gepa_gen3_cand2', 'trial_4'
    VARIANT        VARCHAR,                 -- Flag variant suffix (BASE, AGENTIC, FASTPATH_OFF) or NULL for optimize runs
    ITERATION      VARCHAR,                 -- Iteration name (iter1, iter3) or NULL for flag sweep / GEPA / TPE runs
...
SELECT
    ...
    METRIC_NAME,
    EVAL_AGG_SCORE,
    '<SOURCE>',          -- 'flag_sweep' | 'optimize_dev' | 'optimize_test' | 'gepa_gen' | 'tpe_trial' | 'tpe_final' | 'baseline' | 'revalidation'
    '<OPTIMIZER_TYPE>',  -- 'ITERATIVE' | 'GEPA' | 'INSTRUCTION_TPE' | 'FLAG_SWEEP' | NULL
    '<RUN_ID>',          -- UUID from current OPT_RUNS session; NULL for standalone evals
    '<RUN_LABEL>',       -- e.g. 'FASTPATH_OFF_dev_r1', 'iter3_dev_r2', 'gepa_gen3_cand2', 'trial_4'
    '<VARIANT>',         -- e.g. 'FASTPATH_OFF' or NULL
    '<ITERATION>',       -- e.g. 'iter3' or NULL (NULL for GEPA/TPE — use RUN_LABEL instead)
    '<AGENT_FQN>',
    DURATION_MS,
    LLM_CALL_COUNT,
    ERROR
...
WHERE INPUT_ID = '<INPUT_ID>'
  AND METRIC_NAME = 'answer_correctness'
GROUP BY OPTIMIZER_TYPE, RUN_LABEL, ITERATION, VARIANT
ORDER BY RECORDED_AT;
```

This shows the full journey for a single question across every optimizer type that touched it — flag sweep, iterative, GEPA, DSPy TPE — in chronological order.
...
SELECT
    ...
    MAX(CASE WHEN VARIANT = 'MODEL_A' THEN SCORE END) AS MODEL_A,
    MAX(CASE WHEN VARIANT = 'MODEL_B' THEN SCORE END) AS MODEL_B,
    MAX(CASE WHEN VARIANT = 'MODEL_C' THEN SCORE END) AS MODEL_C
FROM <DATABASE>.<SCHEMA>.<AGENT_NAME>_QUESTION_MANIFEST
...
### 6. Cross-optimizer comparison — "Which optimizer finally fixed this question?"

```sql
SELECT
    INPUT_ID,
    MAX(INPUT_TEXT)                                              AS QUESTION,
    MAX(CASE WHEN OPTIMIZER_TYPE = 'ITERATIVE'      THEN SCORE END) AS ITERATIVE_BEST,
    MAX(CASE WHEN OPTIMIZER_TYPE = 'INSTRUCTION_TPE' THEN SCORE END) AS DSPY_BEST,
    MAX(CASE WHEN OPTIMIZER_TYPE = 'GEPA'           THEN SCORE END) AS GEPA_BEST,
    MAX(SCORE)                                                   AS BEST_EVER,
    MIN(RECORDED_AT)::DATE                                       AS FIRST_SEEN,
    MAX(RECORDED_AT)::DATE                                       AS LAST_SEEN
FROM <DATABASE>.<SCHEMA>.<AGENT_NAME>_QUESTION_MANIFEST
WHERE METRIC_NAME = 'answer_correctness'
  AND OPTIMIZER_TYPE IS NOT NULL
GROUP BY INPUT_ID
ORDER BY BEST_EVER ASC;
```

Questions where `GEPA_BEST` > `DSPY_BEST` and `DSPY_BEST` > `ITERATIVE_BEST` show the
escalation working as intended. Questions where all three are low are governance signal
candidates — route to `pattern-mining.md` Query 2.
...
- **Schema migration:** If adding `OPTIMIZER_TYPE` and `RUN_ID` to an existing table:
  ```sql
  ALTER TABLE <DATABASE>.<SCHEMA>.<AGENT_NAME>_QUESTION_MANIFEST
    ADD COLUMN OPTIMIZER_TYPE VARCHAR,
    ADD COLUMN RUN_ID VARCHAR;
  ```
  Existing rows will have NULL for both — filter with `WHERE OPTIMIZER_TYPE IS NOT NULL`
  to exclude pre-migration rows from cross-optimizer queries.
