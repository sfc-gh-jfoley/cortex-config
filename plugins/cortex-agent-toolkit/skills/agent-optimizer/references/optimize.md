...
> Procedural reference for the `agent-optimizer` skill, loaded by the router in `SKILL.md`. Not independently invokable.
...
**Open Snowflake run record** (load DDL from `../../../../references/pattern-mining.md`):
...
VALUES ('<run_id>', 'ITERATIVE', '<DATABASE>.<SCHEMA>.<AGENT_NAME>', '<SV_FQN>',
        <last_accepted_combined_mean>, 'IN_PROGRESS', CURRENT_TIMESTAMP());
```

Store `<run_id>` in session context for the duration of this iteration.
...
INSERT DEV results per run using the pattern from `references/question-manifest.md` with `SOURCE='optimize_dev'`, `ITERATION='<ITER_NAME>'`, `VARIANT=NULL`. **Mandatory — create the table first if it does not exist.**

Also insert into the shared cross-agent history if `opt_history_fqn` is set in `metadata.yaml` (load INSERT pattern from `../../../../references/pattern-mining.md`):
...
SELECT '<DATABASE>.<SCHEMA>.<AGENT_NAME>', '<SV_FQN>', 'ITERATIVE', '<run_id>',
    NULL, '<primary_operator_applied>',
    INPUT_ID, INPUT, METRIC_NAME, EVAL_AGG_SCORE,
    EVAL_AGG_SCORE - <baseline_score>,
    'iterative_dev', '<ITER_NAME>_dev_r<RUN>', TEST_CATEGORY
...
4b. **SV data quality / schema gap**
   ...
   | Returns data but wrong/partial | SV quality issue — ambiguous descriptions or incorrect metric definitions | Route to `sv-iterative-optimizer` |
...
Options:
  (A) Fix the semantic view first → sv-ddl or sv-iterative-optimizer
...
**⚠️ STOP (INTERACTIVE mode):** Present failure analysis with classifications and proposed instruction changes to the user. In AUTONOMOUS mode: proceed if all failures have a single unambiguous classification; stop and ask if any failure has multiple plausible classifications or if the proposed change touches 3+ files.
...
**⚠️ STOP (INTERACTIVE mode):** Present the proposed instruction changes (diff) and get approval before building. In AUTONOMOUS mode: proceed.
...
INSERT TEST results per run using the pattern from `references/question-manifest.md` with `SOURCE='optimize_test'`, `ITERATION='<ITER_NAME>'`, `VARIANT=NULL`. **Mandatory — create the table first if it does not exist.**

Also insert into `OPT_QUESTION_HISTORY` with `source='iterative_test'` (same pattern as Step 3c with `source='iterative_test'`).

**Log the iteration as a candidate record:**
...
VALUES ('<ITER_NAME>', '<run_id>', NULL, '<primary_operator>', '<target_file>',
        <combined_mean>, <combined_mean> - <baseline_score>, <regression_count>, FALSE);
-- accepted is updated to TRUE in Step 9 if the iteration is accepted
```
...
Close the Snowflake run record (updated to ACCEPT or REJECT after `references/review.md` returns):

```sql
-- After ACCEPT:
UPDATE <DATABASE>.<SCHEMA>.<AGENT_NAME>_OPT_RUNS
SET final_score = <combined_mean>, delta = <combined_mean> - baseline_score,
    winner_operator = '<primary_operator>', generations_run = 1,
    status = 'CONVERGED', ended_at = CURRENT_TIMESTAMP()
WHERE run_id = '<run_id>';

UPDATE <DATABASE>.<SCHEMA>.<AGENT_NAME>_OPT_CANDIDATES
SET accepted = TRUE WHERE candidate_id = '<ITER_NAME>' AND run_id = '<run_id>';

-- After REJECT:
UPDATE <DATABASE>.<SCHEMA>.<AGENT_NAME>_OPT_RUNS
SET final_score = <combined_mean>, delta = <combined_mean> - baseline_score,
    generations_run = 1, status = 'BUDGET_EXHAUSTED', ended_at = CURRENT_TIMESTAMP()
WHERE run_id = '<run_id>';
```
