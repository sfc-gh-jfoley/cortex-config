...
description: >
  ...
  Note: for standalone 3-variant flag comparison use agent-model-tester. This skill's
...
| **FLAG SWEEP** | "flag sweep", "revalidate flags", "re-run flag comparison", "compare EnableAgenticAnalyst", "compare feature flags" | Load `references/flag-sweep.md` and follow its workflow. **Note:** for standalone 3-variant comparison from scratch (no active optimization project), route to `agent-model-tester` instead. |
...
- SV returns wrong/partial data → **SV quality issue**. Route to `sv-iterative-optimizer`.
...
Detect or ask whether to run in **INTERACTIVE** or **AUTONOMOUS** mode:

- **INTERACTIVE** (default): All `⚠️ STOP` gates are active. The user approves each decision before proceeding.
...
Default to INTERACTIVE if the user's preference is unclear.
