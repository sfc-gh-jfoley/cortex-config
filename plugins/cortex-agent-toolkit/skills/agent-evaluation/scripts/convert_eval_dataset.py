...
canonical schema with INPUT_QUERY, GROUND_TRUTH VARIANT, and evaluation metadata.
...
def convert_dataset(
    ...
    if source_table.upper() == target_table.upper():
        raise ValueError("Source and target must be different tables")
    ...
    try:
        ...
        for row in rows:
            ...
            if not isinstance(question, str) or not question.strip():
                raise ValueError("Every evaluation question must be a nonempty string")
        ...
        create_command = "CREATE OR REPLACE" if drop_target else "CREATE"
        cursor.execute(f"""
        {create_command} TABLE {target_table} (
            TEST_ID NUMBER(38,0) AUTOINCREMENT,
            TEST_CATEGORY VARCHAR DEFAULT 'IMPORTED',
            INPUT_QUERY VARCHAR(16777216),
            GROUND_TRUTH VARIANT,
            SPLIT VARCHAR DEFAULT 'VALIDATION'
        )
        """)
        insert_sql = f"INSERT INTO {target_table} (INPUT_QUERY, GROUND_TRUTH) SELECT %s, PARSE_JSON(%s)"
        if converted:
        ...
        cursor.execute(f"SELECT INPUT_QUERY, GROUND_TRUTH FROM {target_table} LIMIT 1")
        sample = cursor.fetchone()
        if sample:
            print(f"\nSample row:")
            q = sample[0]
            print(f"  INPUT_QUERY: {q[:100]}{'...' if len(q) > 100 else ''}")
            print(f"  GROUND_TRUTH: {str(sample[1])[:200]}")
        ...
        print(f"      '{ds_name}',")
        print(f"      OBJECT_CONSTRUCT('query_text', 'INPUT_QUERY', 'expected_tools', 'GROUND_TRUTH')")
        print(f"  );")
...
def main():
    ...
    parser.add_argument("--drop-target", action="store_true", help="Explicitly replace target after source rows are read and validated")
