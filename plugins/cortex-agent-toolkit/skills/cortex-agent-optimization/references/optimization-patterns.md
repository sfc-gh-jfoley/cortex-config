...
LLM-based eval metrics have inherent variance from non-deterministic model responses. A +2% improvement on a single run can easily be noise. Running multiple evals and comparing means reduces the probability of accepting noise as signal or rejecting real improvements. The cost is N× eval time per iteration, which is justified for production agents where a wrong accept/reject decision wastes an entire iteration. See `references/setup.md` for recommended `runs_per_split` values based on dataset size.
