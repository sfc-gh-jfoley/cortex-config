...
| If IS_ENUM is used, SAMPLE_VALUES should be provided (warning, not error) | For dimensions with `IS_ENUM` but no `WITH SAMPLE_VALUES`, warn: "This dimension is marked as an enumeration but has no sample values. Consider adding `WITH SAMPLE_VALUES (...)` to guide AI generation." Proceeding without sample values is allowed but degrades Analyst quality. |
| Dimension with SAMPLE_VALUES but no IS_ENUM is valid but redundant | For dimensions with `WITH SAMPLE_VALUES` but no `IS_ENUM`: flag as informational (not an error): "This dimension provides sample values but is not marked as an enumeration. If values are finite and known, add `IS_ENUM` to improve AI query generation." User may ignore this suggestion. |
