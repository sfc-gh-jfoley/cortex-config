...
> Procedural reference for the `sv-iterative-optimizer` skill. Loaded by the router in `SKILL.md` (EVAL-DATA intent). Not independently invokable.
...
Context: "Creating VQRs to bootstrap sv-iterative-optimizer for <SV_FQN>.
          Target: at least 5 VQRs covering the key business questions for this SV.
          After creation, return to sv-iterative-optimizer to run the eval baseline."
```
...
VQR setup complete:
  ...
  Next step:   Run sv-evaluation to establish baseline, then sv-iterative-optimizer → optimize
```
