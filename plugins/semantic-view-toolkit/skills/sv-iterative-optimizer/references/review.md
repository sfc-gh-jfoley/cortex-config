...
> Procedural reference for the `sv-iterative-optimizer` skill. Loaded by the router in `SKILL.md` (REVIEW intent). Not independently invokable.
...
**STOP GATE (INTERACTIVE mode):** Wait for user selection.

**AUTONOMOUS mode:** follow the computed `Recommendation` above automatically (A/B/C
map to it directly); stop and report after 3 consecutive rejections with no score
improvement rather than looping indefinitely.
