...
- **INTERACTIVE**: Composition plan review gates block for explicit operator confirmation.
- **AUTONOMOUS**: Auto-accepts the non-mutating handoff plan, logs it, and continues. No production mutation occurs in this skill (it produces a plan for hand-off to other skills); the AUTONOMOUS flag propagates to the downstream skill invoked.
...
Produce structured output for `cortex-agent-toolkit`'s `agent-ddl` skill:
...
Multi-SV Agent Composition Plan:
  ...
  Next step: Hand off to $agent-ddl to create the agent.
```

**INTERACTIVE mode:** Wait for operator confirmation before proceeding to Step 4.
```
Proceed? (yes / adjust)
```
**AUTONOMOUS mode:** Auto-accept the composition plan and log it.
`[AUTO-RESOLVED: REARCHITECT-PLAN-REVIEW → accept (non-mutating handoff plan, autonomous mode)]`

### Step 4: Hand off

Provide the structured JSON to the user with instructions:
```
To create this agent, invoke agent-ddl (from cortex-agent-toolkit):
  $agent-ddl
